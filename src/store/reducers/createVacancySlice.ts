...
export type arrayProps = Pick<vacancy,'benefits' | 'qualifications' | "responsibilities" >
...
export const createVacancySlice = createSlice({
    ...
    reducers:{
        ...
        addInArray(state,action:PayloadAction<{field:keyof arrayProps,value:string}>){
            const {field,value} = action.payload;
            state[field] = [...state[field],value]
        },
        removeInArray(state,action:PayloadAction<{field:keyof arrayProps,index:number}>){
